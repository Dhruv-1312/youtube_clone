    ...
    return (
        <div className="overflow-x-hidden relative" ref={containerRef}>
            <div
                ...
                className="flex whitespace-normal gap-3 transition-transform w-[max-content]" style={{ transform: `translateX(-${translate}px)` }}>
                {categories.map((category) => <Button onClick={() => onSelect(category)} variant={selectedCategory == category ? "dark" : "default"}
                    key={category} className="py-1 px-3 rounded-lg whitespace-nowrap">
                    {category}
                </Button>
                )
                }
